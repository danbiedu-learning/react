...
### Dropdown에서 Event Bubbling을 사용

> (Dropdown 바깥을 클릭하면 Dropdown이 닫히도록 하기위해서)

![](./img/jsx_event.png)

- React Component내에 있는 event handler는 셋팅하기 쉽다
  ![](./img/jsx_event_need_bubble.png)
- 하지만 React Component내에 있지 않으면 셋팅하기 위해 다른 방법이 필요하다.
- 예시: Dropdown의 바깥쪽을 누르면 Dropdown이 자동으로 닫혀야 한다. (Dropdown보다 상위에서 event handler가 동작해야함)
  ![](./img/event_bubble.png)
- user가 click하면 event object가 생성됨, DOM에서 bubble up하면서 event handler가 있는 곳을 실행시킴
  ![](./img/onclick_bubbling_sequence.png)
- bubbling이 되면 event handler는 아래 순서로 실행된다. (위 이미지에 순서참고)
  1. addEventListener의 callback function
  2. React Component의callback function
